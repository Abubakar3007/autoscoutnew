...
import { findSingleCar, similarCars } from "@/services/product";
...
export default function ObjectDetails() {
    ...
    const { id } = useParams();
    const car = findSingleCar(id);
    const { handleAddToCart, findCars } = useCart(car);
    ...
    useEffect(() => {
        const fetchData = async () => {
            const data = await similarCars(car?.brand);
            setSimilarCarsData(data);
        }
    ...
    // basic data
    const basicData = [
        { title: "Body shape", value: car?.body_shape },
        { title: "Seats", value: car?.seats },
        { title: "Doors", value: car?.doors },
        { title: "Drive type", value: car?.drive_type },
        { title: "Country version", value: car?.country },
    ];

    // vehicle history
    const vehicleHistoryData = [
        { title: "Mileage", value: car?.mileage },
        { title: "Initial registration", value: car?.registration },
        { title: "Final inspection", value: "04/2022" },
        { title: "Service history maintained", value: "Yes" },
        { title: "Non-smoking vehicle", value: "Yes" },
        { title: "Vehicle owner", value: checkOwnership(car?.ownership) },
    ];
    ...
    const technicalData = [
        ...
        { title: "Gears", value: car?.gears },
        { title: "Cylinder", value: "6" },
    ];

    // power consumption data
    const fuelData = [
        { title: "Fuel", value: car?.fuel_type },
        { title: "Fuel consumption2.8", value: "10.3 l/100 km (combined), 14.9 l/100 km (urban)" },
    ...
    return (
        <section className="py-20">
            <div className="wrapper">
                <button onClick={() => router.push("/listing")} className="text-blue-600 cursor-pointer">
                    Back to Listing
                </button>
                <ObjectHeading
                    title={car.title}
                    price={car.price}
                    location={car.location || "On site"}
                ...
                <div className="flex gap-10">
                    ...
                    <div className="w-full">
                        {
                            car.images.length > 0 && <Gallery carImage={car?.images} title={car?.title} />
                        }
                    ...
                    <div className="max-w-[300px] w-full">
                        ...
                        />
                        {/* dealer contact */}
                        <div className="p-6 mt-10 bg-white rounded-md shadow-box-shadow">
                            <a href="" className="block w-full text-center secondary-btn">
                                <span>+33 (0)3- 12345678</span>
                                <span className="sm:hidden">Call</span>
                            </a>
                            <Button text="Contact" className="block w-full h-12 mt-6" />
                        </div>
                    </div>
                </div>
                {/* Similar cars */}
                <SimilarCars similarCarsData={similarCarsData} />
            </div>
        </section>
    )
}