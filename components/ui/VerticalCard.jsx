...
export default function VerticalCard({ car }) {
    ...
    let findCar = items.find((i) => i.id === car.id);
    ...
    return (
        <div className="relative bg-white rounded-md sm:p-4 card shadow-box-shadow group">
            <div className="flex flex-col sm:gap-4 sm:flex-row">

                {/* car image */}
                <div className="relative w-full overflow-hidden sm:max-w-[250px]">

                    <a href={`/object-details/${car?.id}`} title={car?.title} target="_blank" className="block w-full">
                        <Image
                            src={car?.images?.[1]}
                            alt="object image"
                            width={100}
                    ...
                    <div className="absolute flex gap-2 duration-300 ease-in-out sm:flex-col sm:gap-4 top-3 right-3 sm:-translate-y-1/2 sm:-right-14 sm:top-1/2 trnasition-all sm:group-hover:right-2">
                        ...
                        <button
                            ...
                            className="grid w-8 h-8 bg-white rounded-full cursor-pointer place-items-center"
                        >
                            {
                                findCar ? (
                        ...
                        {/* share button */}
                        <button className="grid w-8 h-8 bg-white rounded-full cursor-pointer place-items-center">
                ...
                <div className="w-full p-4 sm:p-0">
                    ...
                    <h3 className="text-xl font-semibold leading-8 line-clamp-1 text-ellipsis">
                        <a href="details.html" title="Lamborghini Murcielago LP640 E-Ge" className="hover:text-blue-500">{car?.title}</a>
                    </h3>
                    ...
                    <ul className="grid grid-cols-2 mb-4 gap-4 text-sm font-medium [&>li]:pl-8 [&>li]:bg-left [&>li]:bg-no-repeat [&>li]:whitespace-nowrap [&>li]:overflow-hidden [&>li]:text-ellipsis">
                        <li className="km">{car?.kms_driven}</li>
                        ...
                        <li className="user">{car?.seller_type}</li>
                    </ul>

                    {/* price */}
                    <div className="text-xl font-bold leading-8 text-blue-500">{car?.price}</div>
                </div>
            </div>
