...
export default function ListSearchBox({ data, onClick }) {

    let total = Object.values(data).filter(value => value !== "").length;
    console.log(total);
    ...
    return (
        <div className="flex items-center gap-2 p-3 mb-2 lg:p-6 lg:block lg:rounded-md box shadow-box-shadow bg-white">
            ...
            <div className="items-center justify-between hidden pb-3 mb-5 text-base font-semibold border-b lg:flex border-neutral-400">
                <h4 className="leading-6">My search <span>({total})</span></h4>
            ...
            >
                {
                    Object.entries(data).filter(([, value]) => value !== "").map(([key, value]) => (
                        <li key={key}>
